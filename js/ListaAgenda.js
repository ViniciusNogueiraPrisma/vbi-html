...
function excluirEventoDaListEnviarLembrete(e) {
    ...
    var splitCookieEvento = docCookies.getItem('CookieListaAgendaModelo').split(',');
    
    splitCookieEvento = $.grep(splitCookieEvento, function (value) {
        return value != excluirId;        
    });
    docCookies.setItem('CookieListaAgendaModelo', splitCookieEvento);
        
    var splitCookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaModelo").split(',');
    ...
    for (var i = 0; i < splitCookieLembrete.length; i++) {
        if (splitCookieLembrete[i].split('&D')[0] == excluirId) {
            ...
            docCookies.setItem('CookieEnviarLembreteAgendaModelo', splitCookieLembrete);
        }
    }
...
function addEventoAListEnviarLembrete(e) {
    ...
    if (e.currentTarget.checked == true) {
        ...
        var cookieEvento = docCookies.getItem('CookieListaAgendaModelo');
        ...
        if (typeof cookieEvento != 'undefined' && cookieEvento != "" && cookieEvento != null) { // Se cookie já tiver valor
            ...
            if ($.inArray(idEvento, splitCookieEvento) == -1) // Se idEvento NÃO estiver no cookie
            {
                ...
                var cookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaModelo");  //  Adiciona o cookie 
                ...
                docCookies.setItem("CookieEnviarLembreteAgendaModelo", splitCookieLembrete);  //  evento
            ...
            docCookies.setItem("CookieListaAgendaModelo", idEvento);
            docCookies.setItem("CookieEnviarLembreteAgendaModelo", idEvento + '&D' + diasParaLembrete);
...
function MarcarLembrete() {
    ...
    var getCookieLembrete = docCookies.getItem('CookieEnviarLembreteAgendaModelo').split(',');
